...
func TestUnmarshal(t *testing.T) {
	...
	if outAddr[0] != (meta.Address{1}) {
		t.Errorf("expected %x, got %x", meta.Address{1}, outAddr[0])
	}
	...
	if outAddrStruct.A[0] != (meta.Address{1}) {
		t.Errorf("expected %x, got %x", meta.Address{1}, outAddrStruct.A[0])
	}
	...
	if outAddrStruct.B[0] != (meta.Address{2}) {
		t.Errorf("expected %x, got %x", meta.Address{2}, outAddrStruct.B[0])
	}
	if outAddrStruct.B[1] != (meta.Address{3}) {
		t.Errorf("expected %x, got %x", meta.Address{3}, outAddrStruct.B[1])
	}
